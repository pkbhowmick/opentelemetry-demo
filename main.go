...
func main() {
	...
	// This is the equivalent of prometheus.NewCounterVec
	helloCounter, err := meter.Float64Counter("hello", api.WithDescription("hello hit counter"))
	if err != nil {
		log.Fatal(err)
	}

	go serveHelloHandler(helloCounter)
...
func serveHelloHandler(counter api.Float64Counter) {
